...
///get Voucher
$voucherSql = "SELECT voucherNo FROM `purchaseGoodsRawMaterial` where type='salesReturnFinishedGoods' order by id desc";
...
if (isset($_REQUEST["purchaseGoodsMetrialSubmit"])) {
  ...
  $salesReturnFinishedGoods = 'salesReturnFinishedGoods';
  $metrialSql = "INSERT INTO `purchaseGoodsRawMaterial`(`partyAccountName`, `accountName`, `voucherNo`, `date`, `totalAmount`, `discountPer`, `addChargeAmount`, `dudChargeAmount`, `addChargeRemark`, `dudChargeRemark`, `totalBill`, `remark`,`type`) VALUES ('$partyAccountName','$accountName','$voucherNo','$date','$totalAmountBase','$addChargesPercentage','$addChargesAmount','$deductionChargeAmount','$addChargesRemark','$deductionChargeRemark','$totalBill','$remark','$salesReturnFinishedGoods')";
  $metrialResult = $conn->exec($metrialSql);
  if ($metrialResult == true) {
    echo '<script>alert("Data Submited Successfull");location.href="salesReturn.php";</script>';
  } else {
...
document.addEventListener("click", function (event) {
  ...
  // Assuming you're fetching 'length' from the database as well
function showUser1234(str) {
  if (str === "") {
    ...
    // document.querySelector(".length").style.display = "none"; // Hide length field
  ...
  xmlhttp.onreadystatechange = function() {
    if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
      ...
      // Hide rate, unit, and length inputs initially
    //   document.querySelector(".rateInput").style.display = "none";
    //   document.querySelector(".unitInput").style.display = "none";
    //   document.querySelector(".length").style.display = "none"; // Hide length field

      // Show rate, unit, and length inputs when a description is selected
      descriptionSelect.addEventListener("change", function() {
        ...
        if (selectedData) {
          ...
          document.querySelector(".length").value = selectedData.length || ""; // Set length value dynamically

          document.querySelector(".rateInput").style.display = "block";
          document.querySelector(".unitInput").style.display = "block";
          document.querySelector(".length").style.display = "block"; // Show length field
        } else {
          document.querySelector(".rateInput").style.display = "none";
          document.querySelector(".unitInput").style.display = "none";
          document.querySelector(".length").style.display = "none"; // Hide length field
...
function insertData() {
    ...
    xhr.open("POST", editId ? "updateFoodsPurchaseData.php" : "insertFoodsPurchaseData.php", true);
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

    xhr.onreadystatechange = function() {
        if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
            console.log(xhr.responseText); // Log the response from PHP
            //alert(xhr.responseText); // Log the response from PHP
            ...
            // Fetch and show updated data based on voucherNo
            showUser123(voucherNo);
        }
    };

    // Send data to the server for insertion or updating
    xhr.send(
        ...
        "&editId=" + encodeURIComponent(editId) + // Add the edit ID if present
        "&purchaseFinishedGoods=" + encodeURIComponent("salesReturnFinishedGoods") +
...
<head>
  <meta charset="utf-8" />
  <title>Sale return| <?php echo $_SESSION["userName"]; ?> </title>
                    ...
                    <form action="" method="post">
                                ...
                                <div class="table-responsive table-card mt-3 mb-1">
                                  <table class="table align-middle table-nowrap" id="itemTable">
                                    ...
                                    <tbody class="list form-check-all">
                                      ...
                                      <td>
                                        <div class="form-group">
                                          <input type="text" class="form-control length" id="length" name="length" placeholder="Length" style="width: 120px;" />
                                        </div>
                                      </td>
                                      <td>
                                        <div class="form-group">
                      ...
                      <script>
                                              function editRow(event, button) {
                                                ...
                                                document.querySelector('[name="length"]').value = length; // Assuming this is correct
