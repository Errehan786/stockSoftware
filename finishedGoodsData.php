...
if (isset($_POST['group_name']) && !empty($_POST['group_name'])) {
    ...
    $stmt = $conn->query("SELECT * FROM `finishGoods` WHERE groupName = '$groupName'");
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        echo '<tr>
                ...
                <td>' . htmlspecialchars($row['stockLimit']) . '</td>
                <td>' . htmlspecialchars($row['costPrice']) . '</td>
                <td>' . htmlspecialchars($row['Rate']) . '</td>
                <td>
                    <div class="d-flex gap-2">
                        <div class="edit">
                            <a href="editcreateFinishGoods.php?finishedGoodsId=' . $row['id'] . '" class="btn btn-sm btn-success edit-item-btn">Edit</a>
                        </div>
                        <div class="remove">
