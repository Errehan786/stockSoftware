...
///get fnishedgoods Data
$finishedSql = "SELECT * FROM `finishGoods` where id='$_REQUEST[finishedGoodsId]'";
$finishedDetails = $conn->query($finishedSql);
$finishedrow = $finishedDetails->fetch(PDO::FETCH_ASSOC);
...
if (isset($_REQUEST['main_submit'])) {
	...
	$sql = "UPDATE `finishGoods` 
        SET `groupName` = '$group_name', 
            ...
            `Unit` = '$units', 
            `costPrice` = '$cpUnit', 
            `Rate` = '$completeFinalAmount_is', 
            `type` = '$type_name', 
            `factor` = '$factor_name', 
            `strand` = '$stands', 
            `guage` = '$gauge_name', 
            `core` = '$coreIs', 
            `twisting` = '$twisting_name', 
            `coilWt` = '$coilWt', 
            `length` = '$length', 
            `metalWt` = '$metalWt', 
            `drawing` = '$drawing', 
            ...
            `labourCharge` = '$labourPercentage', 
            `pacCharge` = '$packagingCharges', 
            `ratePer` = '$finalProductRate' 
        WHERE `id` = '$_REQUEST[finishedGoodsId]'";

if ($conn->exec($sql)) {
    echo '<script>alert("Data has been updated successfully");window.location = "createFinishGoods.php";</script>';
} else { 
    echo '<script>alert("Data has not been updated");window.location = "createFinishGoods.php";</script>';
}
					...
					<div class="row">
						<div class="col-12">
							<div class="page-title-box d-sm-flex align-items-center justify-content-between">
								<h4 class="mb-sm-0">Finished Goods Edit</h4>
								<div class="page-title-right">
									<ol class="breadcrumb m-0">
										<li class="breadcrumb-item"><a href="./dashboard.php">Dashboard</a> </li>
										<li class="breadcrumb-item active">Finished Goods Edit</li>
									</ol>
								</div>
							</div>
						</div>
				...
				<div class="container-fluid">
					<form action="" method="POST">
											...
											<div class="row">
													...
													<div class="mb-4">
														...
														<select id="vendorList" name="group_name" class="form-select" onChange="createVendor(this.value);getFinishedGoodsData(this.value);" required>
															...
															<?php while ($row = $result->fetch(PDO::FETCH_ASSOC)) { ?>
																<option value="<?php echo $row['name']; ?>" <?php if($finishedrow['groupName'] == $row['name']) echo "selected"; ?>>
												...
												<div class="col-lg-4">
													<div class="mb-3">
														...
														<input type="text" class="form-control" name="description" value="<?php echo $finishedrow['description']; ?>" placeholder="Enter Description" required />
													</div>
												</div>
												<div class="col-lg-4">
													<div class="mb-3">
														<label class="form-label">Opening Stock</label>
														<input type="text" class="form-control" name="stock" id="quantityField" value="<?php echo $finishedrow['openingStock']; ?>" placeholder="Opening stock" required />
													</div>
												</div>
												<div class="col-lg-4">
													<div class=" mb-3">
														<label class="form-label">Stock Limit</label>
														<input type="text" class="form-control" name="stockLimit" value="<?php echo $finishedrow['stockLimit']; ?>" placeholder="Enter Stock Limit" />
													</div>
												</div>
												<div class="col-lg-3" style="padding-right: 0;">
													<div class="mb-4">
														...
														<option value="Kg" <?php if($finishedrow['Unit'] == "Kg")echo "selected"; ?>>Kg</option>
														<option value="Meters" <?php if($finishedrow['Unit'] == "Meters")echo "selected"; ?>>Meters</option>
														<option value="Pcs" <?php if($finishedrow['Unit'] == "Pcs")echo "selected"; ?>>Pcs</option>
														</select>
													</div>
												</div>
												<div class="col-lg-2">
													<div class=" mb-3">
														...
														<input type="text" class="form-control" name="cpUnit" value="<?php echo $finishedrow['costPrice']; ?>" placeholder="Enter Cost Price/Unit" />
													</div>
												</div>
											...
											<div class="table-responsive table-card mt-3 mb-1">
												<table class="table align-middle table-nowrap" id="firstTable">
													...
													<tbody class="list form-check-all">
														<td>
															<div class="form-group">
																...
																<select id="typeList" name="type_name" class="form-select" onChange="createType(this.value)" required>
																	...
																	<hr>
																	<?php while ($row = $result->fetch(PDO::FETCH_ASSOC)) { ?>
																		<option value="<?php echo $row['type']; ?>" <?php if($finishedrow['type'] == $row['type'])echo "selected"; ?>>
															...
															<div class="form-group" id="textarea1">
																<select id="factorList" class="form-select" name="factor_name" style="width:140px"
																	...
																	while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
																		echo '<option value="' . htmlspecialchars($row['name']) . '"';if ($finishedrow["factor"] == $row["name"]) {echo ' selected';}echo '>' . htmlspecialchars($row['name']) . '</option>';
																	}
																	?>
																</select>
															</div>
														...
														<td>
															<div class="form-group">
																<input type="text" class="form-control" name="stands" id="stands" style="width: 100px;" value="<?php echo $finishedrow["strand"]; ?>" placeholder="strand" onInput="calculateMetalWt()" required />
															</div>
														</td>
														<td>
															<div class="form-group" id="">
																<select id="gaugeList" class="form-select" name="gauge_name" style="width:140px" onChange="createGauge(this.value); calculateMetalWt()" required>
																	...
																	$result = $conn->query($sql);
																	while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
																		echo '<option value="' . htmlspecialchars($row['name']) . '"';if ($finishedrow["guage"] == $row["name"]) {echo ' selected';}echo '>' . htmlspecialchars($row['name']) . '</option>';
														...
														<td>
															<div class="form-group" id="">
																<input type="text" class="form-control" name="coreIs" id="coreIs" style="width: 100px;" value="<?php echo $finishedrow["core"]; ?>" placeholder="Core" onInput="calculateMetalWt()" required />
															</div>
														</td>
														<td>
															<div class="form-group">
																<select id="twistingList" class="form-select" name="twisting_name" style="width:140px" onChange="createTwisting(this.value); calculateMetalWt()" required>
																	...
																	$result = $conn->query($sql);
																	while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
																	    echo '<option value="' . htmlspecialchars($row['name']) . '"';if ($finishedrow["twisting"] == $row["name"]) {echo ' selected';}echo '>' . htmlspecialchars($row['name']) . '</option>';
																	}
																	?>
																</select>
															</div>
														...
														<td>
															<div class="form-group">
																<input type="text" id="coilWt" name="coilWt" class="form-control" value="<?php echo $finishedrow["coilWt"]; ?>" placeholder="Coil Wt." style="width: 100px;" required />
															</div>
														</td>
														<td>
															<div class="form-group">
																<input type="text" class="form-control" id="length" name="length" placeholder="Length" value="<?php echo $finishedrow["length"]; ?>" style="width: 100px;" onInput="calculateMetalWt()" required />
															</div>
														</td>

														<td>
															<div class="form-group">
																<input type="text" id="metalWt" name="metalWt" class="form-control" placeholder="Metal Wt." value="<?php echo $finishedrow["metalWt"]; ?>" style="width: 100px;" readonly />
															</div>
														</td>
														<td>
															<div class="form-group" id="">
																<input type="text" class="form-control" name="drawing" name="drawing" id="drawing" placeholder="Drawing" value="<?php echo $finishedrow["drawing"]; ?>" style="width: 100px;" required />
															</div>
														</td>
														<td>
															<div class="form-group">
																<input type="text" class="form-control" name="rRateField" id="rRateField" placeholder="R/Rate" value="<?php echo $finishedrow["rRate"]; ?>" style="width: 100px;" required>
															</div>
														</td>
														</tr>
												...
												<table class="table align-middle table-nowrap mb-0" id="secondTable">
													...
													<tbody class="list form-check-all">
														<tr>
																...
																<div class="form-group" id="textarea1">
																	<select id="iteamList0" class="form-select grade-select" name="grade1" style="width:160px;" required>
																		...
																		<?php foreach ($rows as $row1) { ?>
																			<option value="<?php echo $row1['description']; ?>"<?php if($finishedrow["grade1"] == $row1["description"]) echo "selected";?>><?php echo $row1['description']; ?></option>
																		<?php } ?>
																	</select>
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group" id="">
																	<input type="text" class="form-control avg-rate" name="avgRate1" value="<?php echo $finishedrow["avgRate1"]; ?>" />
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group">
																	<input type="text" class="form-control percentage-field" name="percentage1" value="<?php echo $finishedrow["percentage1"]; ?>" onkeyup="getCalculation(this.value)" placeholder="Percentage" required />
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group">
																	<input type="text" class="form-control weight-field" name="weight1" value="<?php echo $finishedrow["weight1"]; ?>" placeholder="Weight" required />
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group" id="">
																	<input type="text" class="form-control amount-field" name="amount1" value="<?php echo $finishedrow["amount1"]; ?>" placeholder="Amount" required />
																</div>
															</td>
														</tr>
														<tr>
															...
															<td class="border-bottom-0">
																<div class="form-group" id="textarea1">
																	<select id="iteamList0" class="form-select grade-select" name="grade2" style="width:160px;" data-choices data-choices-sorting="true">
																		<option value="">Select Grade</option>
																		<?php foreach ($rows as $row1) { ?>
																			<option value="<?php echo $row1['description']; ?>" <?php if($finishedrow["grade2"] == $row1["description"]) echo "selected";?>><?php echo $row1['description']; ?></option>
																		<?php } ?>
																	</select>
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group" id="">
																	<input type="text" class="form-control avg-rate" name="avgRate2" value="<?php echo $finishedrow["avgRate2"]; ?>" />
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group">
																	<input type="text" class="form-control percentage-field" name="percentage2" value="<?php echo $finishedrow["percentage2"]; ?>" onkeyup="getCalculation(this.value)" placeholder="Percentage" />
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group">
																	<input type="text" class="form-control weight-field" name="weight2" value="<?php echo $finishedrow["weight2"]; ?>" placeholder="Weight" />
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group" id="">
																	<input type="text" class="form-control amount-field" name="amount2" value="<?php echo $finishedrow["amount2"]; ?>" placeholder="Amount" />
																</div>
															</td>
														</tr>
														<tr>
															...
															<td class="border-bottom-0">
																<div class="form-group" id="textarea1">
																	<select id="iteamList0" class="form-select grade-select" name="grade3" style="width:160px;" data-choices data-choices-sorting="true">
																		<option value="">Select Grade</option>
																		<?php foreach ($rows as $row1) { ?>
																			<option value="<?php echo $row1['description']; ?>" <?php if($finishedrow["grade3"] == $row1["description"]) echo "selected";?>><?php echo $row1['description']; ?></option>
																		<?php } ?>
																	</select>
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group" id="">
																	<input type="text" class="form-control avg-rate" name="avgRate3" value="<?php echo $finishedrow["avgRate3"]; ?>" />
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group">
																	<input type="text" class="form-control percentage-field" name="percentage3" value="<?php echo $finishedrow["percentage3"]; ?>" onkeyup="getCalculation(this.value)" placeholder="Percentage" />
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group">
																	<input type="text" class="form-control weight-field" name="weight3" value="<?php echo $finishedrow["weight3"]; ?>" placeholder="Weight" />
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group" id="">
																	<input type="text" class="form-control amount-field" name="amount3" value="<?php echo $finishedrow["amount3"]; ?>" placeholder="Amount" />
																</div>
															</td>
														</tr>
														<tr>
															...
															<td class="border-bottom-0">
																<div class="form-group" id="textarea1">
																	<select id="iteamList0" class="form-select grade-select" name="grade4" style="width:160px;" data-choices data-choices-sorting="true">
																		<option value="">Select Grade</option>
																		<?php foreach ($rows as $row1) { ?>
																			<option value="<?php echo $row1['description']; ?>" <?php if($finishedrow["grade4"] == $row1["description"]) echo "selected";?>><?php echo $row1['description']; ?></option>
																		<?php } ?>
																	</select>
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group" id="">
																	<input type="text" class="form-control avg-rate" name="avgRate4" value="<?php echo $finishedrow["avgRate4"]; ?>" />
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group">
																	<input type="text" class="form-control percentage-field" name="percentage4" value="<?php echo $finishedrow["percentage4"]; ?>" onkeyup="getCalculation(this.value)" placeholder="Percentage" />
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group">
																	<input type="text" class="form-control weight-field" name="weight4" value="<?php echo $finishedrow["weight4"]; ?>" placeholder="Weight" />
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group" id="">
																	<input type="text" class="form-control amount-field" name="amount4" value="<?php echo $finishedrow["amount4"]; ?>" placeholder="Amount"/>
																</div>
															</td>
														</tr>
														<tr>
															...
															<td class="border-bottom-0">
																<div class="form-group" id="textarea1">
																	<select id="iteamList0" class="form-select grade-select" name="grade5" style="width:160px;" data-choices data-choices-sorting="true">
																		<option value="">Select Grade</option>
																		<?php foreach ($rows as $row1) { ?>
																			<option value="<?php echo $row1['description']; ?>" <?php if($finishedrow["grade5"] == $row1["description"]) echo "selected";?>><?php echo $row1['description']; ?></option>
																		<?php } ?>
																	</select>
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group" id="">
																	<input type="text" class="form-control avg-rate" name="avgRate5" value="<?php echo $finishedrow["avgRate5"]; ?>"/>
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group">
																	<input type="text" class="form-control percentage-field" name="percentage5" value="<?php echo $finishedrow["percentage5"]; ?>" onkeyup="getCalculation(this.value)" placeholder="Percentage" />
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group">
																	<input type="text" class="form-control weight-field" name="weight5" value="<?php echo $finishedrow["weight5"]; ?>" placeholder="Weight" />
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group" id="">
																	<input type="text" class="form-control amount-field" name="amount5" value="<?php echo $finishedrow["amount5"]; ?>" placeholder="Amount" />
																</div>
															</td>
														</tr>
														<tr>
															...
															<td class="border-bottom-0">
																<div class="form-group" id="textarea1">
																	<select id="iteamList0" class="form-select grade-select" name="grade6" style="width:160px;" data-choices data-choices-sorting="true">
																		<option value="">Select Grade</option>
																		<?php foreach ($rows as $row1) { ?>
																			<option value="<?php echo $row1['description']; ?>" <?php if($finishedrow["grade6"] == $row1["description"]) echo "selected";?>><?php echo $row1['description']; ?></option>
																		<?php } ?>
																	</select>
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group" id="">
																	<input type="text" class="form-control avg-rate" name="avgRate6" value="<?php echo $finishedrow["avgRate6"]; ?>" />
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group">
																	<input type="text" class="form-control percentage-field" name="percentage6" value="<?php echo $finishedrow["percentage6"]; ?>" onkeyup="getCalculation(this.value)" placeholder="Percentage"/>
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group">
																	<input type="text" class="form-control weight-field" name="weight6" value="<?php echo $finishedrow["weight6"]; ?>" placeholder="Weight"/>
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group" id="">
																	<input type="text" class="form-control amount-field" name="amount6" value="<?php echo $finishedrow["amount6"]; ?>" placeholder="Amount"/>
																</div>
															</td>
														</tr>
														<tr>
												...
												<table class="table align-middle table-nowrap mb-0" id="customerTable">
													<thead class="table-light">
														<tr>
															<th>PVC Wt. </th>
															<th class="d-flex">
																<input type="text" class="form-control inline-block w-50 me-1" name="pvcWt" id="pvcWt" placeholder="PVC Wt." value="<?php echo $finishedrow["PVCWt"]; ?>" required />
																<input type="text" class="form-control inline-block w-50" name="avrateTotal" id="totalAmount" placeholder="Total Amount" value="<?php echo $finishedrow["avjWt"]; ?>" readonly />
															</th>
														</tr>
													</thead>
													<tbody class="list form-check-all">
														<tr>
															...
															<td class="border-bottom-0">
																<div class="form-group" id="">
																	<input type="text" class="form-control" name="finalTotalAmount" id="finalTotalAmount" value="<?php echo $finishedrow["totalAmount"]; ?>"/>
																</div>
															</td>
														</tr>
														<tr>
															<td class="border-bottom-0">
																<div class="form-group d-flex" id="">
																	...
																	<input type="text" class="form-control w-25 labour-percentage" name="labourPercentage" id="labourPercentage" value="<?php echo $finishedrow["labourCharge"]; ?>" />
																	<p class="ms-1 mb-0 align-content-center">%</p>
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group" id="">
																    <?php $totalLaborAmountIs = ($finishedrow["totalAmount"]/100)*$finishedrow["labourCharge"]; ?>
																	<input type="text" class="form-control totalLaborAmount" id="totalLaborAmount" value="<?php echo $totalLaborAmountIs ?>" placeholder="Total Labour Amount" readonly />
																</div>
															</td>
														</tr>
														...
														<tr>
															...
															<td class="border-bottom-0">
																<div class="form-group" id="">
																	<input type="text" class="form-control" id="packagingCharges" name="packagingCharges" value="<?php echo $finishedrow["pacCharge"]; ?>"/>
																</div>
															</td>
														</tr>
														<tr>
															<td class="border-bottom-0">
																<div class="form-group d-flex" id="">
																	...
																	<input class="form-control" type="text" id="finalProductRate" name="finalProductRate" style="width: 50px;" value="<?php echo $finishedrow["ratePer"]; ?>">
																	<p class="ms-1 mb-0 align-content-center">/Wt</p>
																</div>
															</td>
															<td class="border-bottom-0">
																<div class="form-group" id="">
																    <?php $completeFinalAmount = $finishedrow["totalAmount"] + $totalLaborAmountIs + $finishedrow["pacCharge"]; ?>
																	<input type="text" class="form-control" name="completeFinalAmount" id="completeFinalAmount" value="<?php echo $completeFinalAmount; ?>" readonly />
																</div>
																<div class="form-group" id="">
																	<input type="text" class="form-control" name="completeFinalAmount_is" id="completeFinalAmount_is" value="<?php echo number_format($completeFinalAmount/$finishedrow["ratePer"], 2); ?>" readonly />
																</div>
															</td>
														</tr>
													</tbody>
						...
						<div class="modal-footer">
							<button type="submit" name="main_submit" class="btn btn-info">Update</button>
						</div>
										</div>
									</div>
